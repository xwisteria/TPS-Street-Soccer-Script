...
function Luna:CreateWindow(WindowSettings)
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			Icon = 1,
			SupportedExecutors = {"Vega X", "Delta", "Bunni", "Velocity"}, -- THESE DEFAULTS ARE PLACEHOLDERS!! I DO NOT ADVERTISE THESE, THEY ARE JUS THE FIRST THAT CAME TO MIND. I HAVE NO IDEA WHETHER THEYA RE RATS (they prob are) AND IM NOT RESPONSIBLE IF U GET VIRUSES FROM INSTALLING AFTER SEEING THIS LIST
...
local function rgbBoxes(box, toChange)
    ...
    -- Clamp value first
    if SliderSettings and SliderSettings.Range then
        value = math.clamp(value, SliderSettings.Range[1], SliderSettings.Range[2])
    end

    local color = Color3.fromHSV(h, s, v)
    local oldR = math.floor(color.R*255 + 0.5)
    local oldG = math.floor(color.G*255 + 0.5)
    local oldB = math.floor(color.B*255 + 0.5)
    ...
    color = Color3.fromRGB(oldR, oldG, oldB)
    -- Apply 'color' to your UI element or object here
end
